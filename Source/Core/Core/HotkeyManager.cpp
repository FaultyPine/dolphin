...
constexpr std::array<const char*, NUM_HOTKEYS> s_hotkey_labels{{
    ...
    _trans("Rollback Save State"),
    _trans("Rollback Load State"),
}};
// clang-format on
...
constexpr std::array<HotkeyGroupInfo, NUM_HOTKEY_GROUPS> s_groups_info = {
    ...
    {{_trans("General"), HK_OPEN, HK_REQUEST_GOLF_CONTROL},
     ...
     {_trans("Rollback"), HK_ROLLBACK_SAVE_STATE, HK_ROLLBACK_LOAD_STATE}}};
...
void HotkeyManager::LoadDefaults(const ControllerInterface& ciface)
{
  ...
  set_key_expression(HK_ROLLBACK_SAVE_STATE, hotkey_string({"Ctrl", "F5"}));
  set_key_expression(HK_ROLLBACK_LOAD_STATE, hotkey_string({"Ctrl", "F9"}));
}
