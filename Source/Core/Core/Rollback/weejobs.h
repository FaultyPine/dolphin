...
#define WEEJOBS_VERSION_NUMBER                                                                     \
  WEEJOBS_COMPUTE_VERSION(WEEJOBS_VERSION_MAJOR, WEEJOBS_VERSION_MINOR, WEEJOBS_VERSION_REV)
#define WEEJOBS_VERSION_STRING                                                                     \
  WEEJOBS_STR(WEEJOBS_VERSION_MAJOR)                                                               \
  "." WEEJOBS_STR(WEEJOBS_VERSION_MINOR) "." WEEJOBS_STR(WEEJOBS_VERSION_REV)
...
public:
  ...
  {
    _cv.wait_for(_m, std::chrono::seconds(1),
                 [this, c]() { return (_count == 0) || (c && c->canceled()); });
    _count = 0;
  }
...
{
  static std::shared_ptr<jobgroup> create() { return std::make_shared<jobgroup>(); }
};
...
public:
  ...
  const T& value() const { return std::get<T>(_shared->_obj); }

  //! Dereference this object to const pointer to the result.
  const T* operator->() const { return &std::get<T>(_shared->_obj); }

  //! Result is available AND equal to the argument.
  bool has_value(const T& arg) const { return available() && value() == arg; }
  ...
  const T& join() const
  {
    while (!empty() && !_shared->_ev.wait(std::chrono::milliseconds(1)))
      ;
    return value();
  }
  ...
  //! or a cancelation flag is set; then returns the result object. Be sure to
  //! check canceled() after calling join() to see if the return value is valid.
  const T& join(const cancelable& p) const { return join(&p); }

  //! Release reference to a promise, resetting this future to its default state
  void abandon() { _shared.reset(new shared_t()); }
  ...
  unsigned refs() const { return _shared.use_count(); }
  ...
  template <typename R>
  WEEJOBS_NO_DISCARD inline future<R> then_dispatch(std::function<void(const T&, future<R>&)> func,
                                                    const context& con = {});
...
public:
  //! Destroy
  ~jobpool() { stop_threads(); }

  //! Name of this job pool
  const std::string& name() const { return _metrics.name; }

  metrics_t* metrics() { return &_metrics; }
  ...
  unsigned concurrency() const { return _target_concurrency; }

  //! Whether this job pool is allowed to steal work from other job pools
  //! when it is idle. Default = true.
  void set_can_steal_work(bool value) { _can_steal_work = value; }
  ...
  inline bool _take_job(detail::job& output, bool lock)
  {
    if (lock)
    {
      std::lock_guard<std::mutex> _lock(_queue_mutex);
    ...
    else if (!_done && !_queue.empty())
    {
      ...
      {
        float priority = iter->ctx.priority != nullptr ? iter->ctx.priority() : 0.0f;
...
public:
  ...
  inline const std::vector<struct jobpool::metrics_t*> all() { return _pools; }
...
public:
  ...
  //! @return Future result of the async function call
  template <typename F, typename T = typename detail::result_of_t<F, cancelable&>>
  WEEJOBS_NO_DISCARD inline future<T> dispatch(F task, future<T> promise,
...
inline void runtime::pool_dispatch(std::function<bool()> delegate, const context& context)
{
  ...
  if (pool)
  {
    ...
    if (_stealing_allowed)
    {
      ...
      for (auto pooliter : _pools)
      {
        pooliter->_block.notify_all();
      }
    }
  }
}
...
{
  auto delegate = [task]() mutable -> bool {
    task();
    return true;
  };
...
template <typename F, typename T>
WEEJOBS_NO_DISCARD inline future<T> runtime::dispatch(F task, future<T> promise,
                                                      const context& context)
{
  bool can_cancel = context.can_cancel;
...
inline jobpool::jobpool(runtime& runtime, const std::string& name, unsigned concurrency)
    : _runtime(runtime), _target_concurrency(concurrency)
{
  _metrics.name = name;
...
inline future<R> future<T>::then_dispatch(runtime& rt, F func, const context& con)
{
  ...
  // In this case, the continuation task will return a value that the system will use to resolve the
  // promise.
  ...
  // lock the continuation and set it:
  {
    ...
    _shared->_continuation = [rt, func, copy_of_con, weak_shared, continuation_promise]() mutable {
      ...
      if (shared && shared->_ev.isSet())
      {
        ...
        // Once this wrapper gets created, note that we now have 2 refereces to the
        // continuation_promise. To prevent this from hampering cancelation, the continuation
        // fuction is set to nullptr immediately after being called.
...
template <typename T>
template <typename R>
inline future<R> future<T>::then_dispatch(std::function<void(const T&, future<R>&)> func,
                                          const context& con)
{
...
template <typename T>
inline void future<T>::then_dispatch(runtime& rt, std::function<void(const T&)> func,
                                     const context& con)
{
  // lock the continuation and set it:
  {
